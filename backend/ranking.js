...
function calculateWalletValueInBase({ baseBalance, productBalances, productPrices }) {
  let total = baseBalance;
  for (const productAddress of Object.keys(productBalances)) {
    total += (productBalances[productAddress] || 0) * (productPrices[productAddress] || 0);
  }
  return total;
}

export function calculateRanking({ traders, baseBalances, productBalancesByTrader, productPrices, initialBaseBalance }) {
  const ranking = traders.map((traderAddress) => {
    ...
    const totalValue = calculateWalletValueInBase({ baseBalance, productBalances, productPrices });
    const pnl = totalValue - initialBaseBalance;

    return { trader: traderAddress, baseBalance, productBalances, totalValue, pnl };
  });
